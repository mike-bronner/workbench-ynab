...
| `SLOT:footer-persona` | `bash "${CLAUDE_PLUGIN_ROOT}/bin/persona.sh" html-name` — the resolved persona name, already HTML-escaped by the shared, tested `html_escape` (bin/html-escape.sh). Inject verbatim; never hand-escape the raw `name` yourself (one escape function, not an LLM re-implementation). |
