---
description: Run the annual YNAB review ad-hoc — orchestrator plans first, then the annual tier-wrapper skill runs, forced to the annual tier only. Strictly read-only.
---

The user invoked `/workbench-ynab:ynab-annual-review`. Run the **annual** tier
...
force the execution tier to `annual` — whatever the plan lists, this command
runs exactly one tier. Keep
everything else: the annual `window` from `plan.report.reasons`, `warnings`,
...
## Phase 3 — Execute the annual review

Read the annual tier wrapper at
`${CLAUDE_PLUGIN_ROOT}/skills/review/annual-ynab-review.md` and follow it,
handing over the plan block with `tier = annual` and the plan's annual window.
...
- **Annual only.** Do not run other tiers, whatever the plan block lists — the
