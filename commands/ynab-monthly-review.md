---
description: Run the monthly YNAB review ad-hoc — orchestrator plans first, then the monthly tier-wrapper skill runs, forced to the monthly tier only. Strictly read-only.
---

The user invoked `/workbench-ynab:ynab-monthly-review`. Run the **monthly**
tier of the YNAB review on demand, regardless of what today's schedule says.
...
force the execution tier to `monthly` — whatever the plan lists, this command
runs exactly one tier. Keep
everything else: the monthly `window` from `plan.report.reasons`, `warnings`,
...
## Phase 3 — Execute the monthly review

Read the monthly tier wrapper at
`${CLAUDE_PLUGIN_ROOT}/skills/review/monthly-ynab-review.md` and follow it,
handing over the plan block with `tier = monthly` and the plan's monthly
window. The wrapper defers to the universal protocol, which writes the report
and emits the dispatch summary.
...
- **Monthly only.** Do not run other tiers, whatever the plan block lists — the
