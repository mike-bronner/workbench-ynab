...
| `_cfg_tax_year` | The **optional** active-tax-year override (`.tax_year`). Echoes nothing and returns **zero** when the key is absent — that is the default, and the caller derives the year from the review date. On a value that is not a JSON **number** of exactly four digits it prints a descriptive error to **stderr** and returns **non-zero**; it never ignores a malformed value. Resolve it as a hard stop: `tax_year="$(_cfg_tax_year)" \|\| exit 1`. |
