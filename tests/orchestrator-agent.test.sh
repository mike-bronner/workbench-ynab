...
# Pin the two new warning kinds by their literal `kind` identifiers — not just
# the surrounding prose — matching the sibling anomaly kinds below (missed_weekly
# et al.). A rename or removal of either kind must fail here, exactly as it would
# for the anomaly kinds.
assert_present    "today_missing warning kind is pinned by name"    "today_missing"
assert_present    "timezone_missing warning kind is pinned by name" "timezone_missing"
