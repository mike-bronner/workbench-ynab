...
# --- the active tax-year label, issue #17 ------------------------------------
# The header must say which tax year the figures apply to, and that label must be
# the one the tax engine produced (meta.taxYearLabel from resolveTaxYear) — not a
# year someone read off a budget name. The writer enforces that by SHAPE: only the
# two labels the engine emits are accepted.
test_tax_year_label_is_rendered_into_the_header() {
  local out
  out="$(run_writer_fixture --tier Quarterly-Tax --date 2026-01-05 \
           --output-dir "$SANDBOX/taxyear" --tax-year 'Tax Year 2026 (2025 close-out through 2026-01-15)')"
  assert_contains "$(cat "$out")" 'Tax Year 2026 (2025 close-out through 2026-01-15)' \
    "the changeover label reaches the rendered header"
  # The placeholder itself must be GONE — a surviving {{tax_year}} would mean the
  # substitution never ran and the assertion above passed on the wrong thing.
  case "$(cat "$out")" in
...
test_tax_year_is_optional_and_renders_empty() {
  ...
  local out
  out="$(run_writer_fixture --tier Weekly --date 2026-06-22 --output-dir "$SANDBOX/taxyear-none")"
  case "$(cat "$out")" in
    *'{{tax_year}}'*) fail "an omitted --tax-year left the placeholder in the report" ;;
  esac
  assert_contains "$(cat "$out")" '<p class="meta"></p>' "the omitted label renders as an empty slot"
}

test_off_shape_tax_year_label_is_refused() {
  # Each of these is a way the label could stop coming from resolveTaxYear: a bare
  # year, a budget name, a hand-written variant, a half-formed changeover label, or
  # markup. All must be usage errors, with nothing written.
  local bad rc err
  for bad in '2026' 'Personal 2024' 'Tax year 2026' 'Tax Year 26' 'Tax Year 2026 (2025 close-out)' \
             'Tax Year 2026 (2025 close-out through 2026-13-45)' '<b>Tax Year 2026</b>' 'Tax Year 2026 extra'; do
    rc=0
    err="$(run_writer_fixture --tier Quarterly-Tax --date 2026-01-05 \
             --output-dir "$SANDBOX/taxyear-bad" --tax-year "$bad" 2>&1)" || rc=$?
    assert_eq "2" "$rc" "off-shape --tax-year '$bad' → usage error (exit 2)"
    assert_contains "$err" "invalid --tax-year" "error names the bad tax-year label for '$bad'"
  done
  [ -d "$SANDBOX/taxyear-bad" ] && fail "a refused --tax-year still created an output dir"
  return 0
}
