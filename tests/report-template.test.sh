...
BASELINE="${REPO_ROOT}/docs/a11y-baseline.md"
...
# assert_baseline <desc> <needle> — docs/a11y-baseline.md must contain <needle>.
assert_baseline() {
  local desc="$1" needle="$2"
  if grep -qF -- "$needle" "$BASELINE" 2>/dev/null; then
    printf 'ok   — %s\n' "$desc"; pass=$((pass + 1))
  else
    printf 'FAIL — %s: %q not found\n' "$desc" "$needle"; fail=$((fail + 1))
  fi
...
# The print force-open rule must spare the toggle: only non-summary children are
# forced visible, and summary's display is never suppressed anywhere.
assert_present "a11y: print force-open rule excludes the summary toggle" "details > *:not(summary) { display: block !important; }"
assert_absent  "a11y: summary display is never suppressed" "summary { display: none"

# The baseline doc must exist AND enumerate every rule — a bare existence check
# would let an emptied checklist ship unnoticed. Mirror the slots pattern: pin
# the count, then pin each rule's distinguishing phrase as an unchecked checkbox.
if [ -f "$BASELINE" ]; then
...
checkbox_count=$(grep -c '^- \[ \]' "$BASELINE" 2>/dev/null)
if [ "${checkbox_count:-0}" -eq "${#baseline_rules[@]}" ]; then
  printf 'ok   — a11y: baseline enumerates exactly %d checklist rules\n' "${#baseline_rules[@]}"; pass=$((pass + 1))
else
  printf 'FAIL — a11y: baseline has %s checkbox rules, expected %d\n' "${checkbox_count:-0}" "${#baseline_rules[@]}"; fail=$((fail + 1))
fi
for rule in "${baseline_rules[@]}"; do
  assert_baseline "a11y baseline rule pinned: $rule" "- [ ] $rule"
done
