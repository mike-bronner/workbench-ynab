...
/**
 * Expire fired-alert keys whose condition has CLEARED — the other half of the
 * M6-3 dedupe seam (recordFiredAlert adds; this removes). When a condition an
 * earlier poll announced is no longer true (an account back above its floor, a
 * category back under its overrun threshold), its key must leave the ledger so a
 * future recurrence re-alerts instead of being suppressed forever.
 *
 * A key is DROPPED only when both hold: (1) it is not in `keepKeys` — the set of
 * conditions still active this pass — and (2) its `{type}:` prefix is in
 * `options.types`, when that allow-list is given. The type gate is what lets a
 * caller expire only FULL-DOMAIN condition types (overdrawn, budget_overrun,
 * bill_due — re-evaluated in whole every poll) while preserving POINT-EVENT keys
 * (a large-transaction key names a transaction that never un-happens, and the
 * incremental-cursor window can't attest it cleared). Omit `options.types` to
 * make every type eligible for expiry. Pure — returns a new state, never mutates.
 *
 * @param {object} state    the current (normalized) snapshot.
 * @param {Iterable<string>} keepKeys  dedupe keys whose condition is still active.
 * @param {object} [options]
 * @param {string[]} [options.types]  type-prefixes eligible for expiry; when
 *   omitted, all types are eligible.
 * @returns {{ state: object, expired: string[] }}  `expired` lists the dropped keys.
 */
export function expireFiredAlerts(state, keepKeys, options = {}) {
  const keep = keepKeys instanceof Set ? keepKeys : new Set(keepKeys ?? []);
  const types = options.types;
  const firedAlerts = {};
  const expired = [];
  for (const [key, payload] of Object.entries(state.firedAlerts)) {
    const eligible = !types || types.includes(key.split(':')[0]);
    if (eligible && !keep.has(key)) {
      expired.push(key);
      continue;
    }
    firedAlerts[key] = payload;
  }
  return { state: { ...state, firedAlerts }, expired };
}
...
export function writeState(state, options = {}) {
  ...
  // would stay world-traversable. chmodSync re-tightens it unconditionally, so
  // monitor-state's dir holds the invariant exactly as lib/monitor/alerts.mjs and
  // bin/audit-log.sh do for this same data class (they chmod the dir on every
  // write). A pre-existing PARENT above the leaf keeps its own mode; force-chmod-ing
  // dirs we don't own would overreach, so we stop at the leaf.
