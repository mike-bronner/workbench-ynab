 ...
 * a throw (zero-config requirement). The config PATH is the exception (#169 →
 * #206 → #244): a resolved path escaping the data-dir root (`options.dataDir` →
 * env `YNAB_DATA_DIR` → the canonical plugin-data dir) throws a structured
 * `containment` error before any read, exactly as loadThresholds does.
 *
 * dispatchAlerts, whose NEVER-throws contract can't surface that throw, catches
 * it and degrades to its no-dispatch result — so a hostile YNAB_CONFIG_FILE can
 * refuse a config load without crashing an unattended monitor pass.
 *
...
export function loadAlertsConfig(options = {}, env = process.env) {
  ...
  // Path containment (#169 → #206 → #244): the data dir names the allowlist
  // root — an explicit configFile never vouches for itself (see
  // lib/containment.mjs). Checked OUTSIDE the try below so a containment failure
  // THROWS rather than degrading to defaults; mirrors loadThresholds
  // (lib/tax/confidence.mjs:134).
  const dataDir = options.dataDir ?? env.YNAB_DATA_DIR ?? join(homedir(), DATA_DIR_REL);
  assertContained('the plugin config', configFile, resolveRoots([dataDir]));
  let parsed;
  try {
...
// Containment allowlist for the alert log (#169 → #206 → #244): the resolved
// data dir — options.dataDir → env YNAB_DATA_DIR → the canonical plugin-data
// dir. Naming a root is an embedding-level trust decision (the test seam), so
// an explicit dataDir joins the allowlist WITHOUT widening the default
// no-options surface; an explicit logPath never vouches for itself. An
// unresolvable root is dropped by resolveRoots (fail closed — every access then
// refused). Mirrors lib/monitor/state.mjs stateRoots.
 ...
 *
 * The resolved path is containment-checked (#169 → #206 → #244) before any
 * mkdir/append — an escaping logPath / YNAB_ALERT_LOG_FILE is refused with a
 * structured `containment` write error and nothing is created (no dir, no
 * file), mirroring lib/monitor/state.mjs writeState. Returns the path written.
 *
 * @throws {Error} `code: 'containment'` when the resolved log path
 *   canonicalizes outside the data-dir root — nothing is created.
 */
export function appendAlertLog(entry, options = {}) {
  ...
  // mkdir/append to a caller-supplied path is an arbitrary-path WRITE primitive.
  // Refused before mkdir/append — nothing is created on an escaping path.
  assertContained('the monitor alert log', path, alertLogRoots(options), 'write');
 ...
 *     complete no-op (`dispatched: false`). A config PATH that escapes the data
 *     root (a hostile/misconfigured YNAB_CONFIG_FILE) is refused by
 *     loadAlertsConfig's containment guard; that throw is caught here and
 *     degrades to the SAME no-op shape rather than crashing the pass.
...
export function dispatchAlerts(findings, options = {}) {
  // loadAlertsConfig throws a structured `containment` error when the resolved
  // config path escapes the data root (a hostile/misconfigured YNAB_CONFIG_FILE).
  // dispatchAlerts NEVER throws — an unattended monitor pass must survive a bad
  // config path — so that refusal degrades to the same safe no-dispatch shape as
  // a disabled config, exactly as the appendAlertLog failure below degrades.
  // Diagnostic to stderr only (stdout discipline). A pre-loaded options.config
  // bypasses the load and this catch entirely.
  let config;
  try {
    config = options.config ?? loadAlertsConfig(options, options.env ?? process.env);
  } catch (err) {
    process.stderr.write(`[alerts] config load refused (${err.code ?? 'error'}), not dispatching: ${err.message}\n`);
