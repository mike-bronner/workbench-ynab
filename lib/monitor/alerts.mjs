 ...
 * trail. An unknown severity renders as 🟢 (informational), and a malformed
 * element (null / undefined / a primitive) is skipped before it can consume a
 * cap slot — findings come from detectors this module doesn't control, so a
 * bad element must degrade, never fail an unattended pass. Pure — no IO.
 */
...
export function dispatchAlerts(findings, options = {}) {
  ...
  // Malformed elements (null / undefined / primitives) are dropped up front:
  // findings come from detectors this module doesn't control, and one bad
  // element must never consume a cap slot, pollute the audit log, or — worst —
  // throw in rendering and kill an unattended pass (the NEVER-throws contract).
  const valid = Array.isArray(findings) ? findings.filter(isFinding) : [];
