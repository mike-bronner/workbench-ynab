...
// component is verified TRULY ABSENT (lstat ENOENTs too, see below): whatever
// verdict containment reaches, opening the raw path can only ENOENT too, so no
// bytes can leak through a fabricated suffix.
//
// The lstat check is what makes that claim hold (#206 review): realpath ENOENTs
// for a component that nonetheless EXISTS as a dangling symlink, and re-attaching
// that component's raw name would report the symlink's own (in-root) spelling
// while open(2) on the raw path FOLLOWS the link — a direct write would create
// the symlink's target outside every root. An ENOENT'd component that lstat can
// still see is therefore unresolvable, not absent: fail closed.
//
...
export function canonicalize(p) {
  ...
  let suffix = '';
  for (;;) {
    // realpath ENOENT'd for `prefix` — but only a TRULY absent entry may be
    // re-attached. An entry lstat can still see is a dangling symlink:
    // existing, yet unresolvable to any real target. Fail closed.
    try {
      lstatSync(prefix);
      return null; // exists but realpath-unresolvable (dangling symlink) — fail closed
    } catch (err) {
