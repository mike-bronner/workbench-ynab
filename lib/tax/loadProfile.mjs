...
export function loadProfile(options = {}) {
  ...
  try {
    ...
    // redact: Node's err.message embeds the raw path too, so mask the whole string.
    throw new Error(redact(`tax loader: cannot read bundled default ruleset at ${defaultsPath}: ${err.message}`));
  }
  ...
  if (existsSync(profilePath)) {
    ...
    try {
      ...
      // redact the COMPOSED string: Node's err.message embeds the raw path too.
      return failure('io', [mkErr('', 'io', redact(`cannot read tax profile at ${profilePath}: ${err.message}`), {})], sources);
    }
    try {
      userRaw = JSON.parse(text);
    } catch (err) {
      return failure('parse', [mkErr('', 'parse', redact(`invalid JSON in tax profile at ${profilePath}: ${err.message}`), {})], sources);
    }
    ...
    try {
      ...
      throw new Error(redact(`tax loader: cannot read tax-profile schema at ${schemaPath}: ${err.message}`));
    }
