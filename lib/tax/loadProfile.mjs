...
// existing ancestor natively, and re-attach the untraversed suffix (that suffix
// is symlink-free, since every component in it ENOENT'd, so join's lexical `..`
// handling is safe there).
//
...
// The canonical (realpath'd) home directory, redacted out of any path echoed in
// a containment error so the message never leaks the OS username / plugin-data
// path. Computed once; null only if home itself cannot be resolved.
const HOME_REAL = canonicalize(homedir());
...
export function loadProfile(options = {}) {
  ...
  // resolve rather than admitting paths under an unresolvable one.
  ...
  // path with the homedir redacted, and drop the absolute resolved roots from the
  // human-readable message (they survive, redacted, in the structured detail).
  const redact = (s) => (typeof s !== 'string' ? s : (HOME_REAL ? s.split(HOME_REAL).join('~') : s));
