...
// missing for them.
function stampLeaves(value, path, tier, prov) {
  if (isPlainObject(value)) {
    const keys = Object.keys(value).filter((k) => !isDangerousKey(k));
    if (keys.length === 0) {
      prov[path] = tier;
      return;
    }
    for (const k of keys) stampLeaves(value[k], childKey(path, k), tier, prov);
    ...
    value.forEach((item, i) => stampLeaves(item, indexKey(path, entity ? item.id : i), tier, prov));
  } else {
    prov[path] = tier;
  }
}
...
function mergeLayer(base, layer, tier, prov, path) {
  for (const key of Object.keys(layer)) {
    if (isDangerousKey(key)) continue; // never merge a prototype-pollution key
    const p = childKey(path, key);
    const bv = base[key];
    const lv = layer[key];

    if (isPlainObject(bv) && isPlainObject(lv)) {
      mergeLayer(bv, lv, tier, prov, p);
      ...
      for (const item of lv) {
        ...
        if (existing) {
          mergeLayer(existing, item, tier, prov, ip);
        } else {
          const clone = deepClone(item);
          bv.push(clone);
          byId.set(item.id, clone);
          stampLeaves(clone, ip, tier, prov);
        }
      }
    } else if (!deepEqual(bv, lv)) {
      clearProvUnder(prov, p);
      base[key] = deepClone(lv);
      stampLeaves(base[key], p, tier, prov);
    }
  ...
  return {
    ...
    getQuarterlyDueDates(year) {
      ...
      return (profile.quarterlyEstimatedDueDates ?? []).map((d) => {
        ...
        return { quarter: d.quarter, month: d.month, day: d.day, year: calendarYear, date: `${calendarYear}-${mm}-${dd}` };
      });
    },
  };
}
...
export function loadProfile(options = {}) {
  ...
  const { profile, provenance, defaultsOnly } = resolveProfile(defaultsRaw, userRaw);
  deepFreeze(profile);
