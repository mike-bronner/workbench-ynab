...
// Canonicalize a path the way the kernel resolves it for open(2). For an
// existing target, realpath the RAW path — the kernel resolves symlinks before
// applying `..`, so a lexical normalization first (path.resolve) would resolve
// `link/../x` to the wrong file. For a not-yet-existing target, realpath the
// deepest existing ancestor of the lexically-resolved path and re-attach the
// untraversed suffix: the lexical shortcut is safe there because a read of a
// missing path fails ENOENT regardless — no bytes can be disclosed.
function canonicalize(p) {
  try {
    return realpathSync(p);
  } catch {
    let prefix = resolve(p);
    let suffix = '';
    for (;;) {
      const parent = dirname(prefix);
      if (parent === prefix) return join(prefix, suffix); // reached the fs root
...
export function loadProfile(options = {}) {
  ...
  // else the structured failure to hand straight back to the caller.
  const roots = [canonicalize(dataDir), canonicalize(ASSETS_TAX)];
  const containmentFailure = (label, p) => {
    const real = canonicalize(p);
    if (roots.some((root) => isWithin(root, real))) return null;
    return failure(
      'containment',
      [mkErr('', 'containment', `refusing to read ${label} at ${p}: it resolves to ${real}, outside the allowed roots (${roots.join(', ')})`, { path: real, roots })],
      sources,
    );
  };
  ...
  try {
    ...
    throw new Error(`tax loader: cannot read bundled default ruleset at ${defaultsPath}: ${err.message}`);
  }
  ...
  if (existsSync(profilePath)) {
    ...
    try {
      ...
      return failure('io', [mkErr('', 'io', `cannot read tax profile at ${profilePath}: ${err.message}`, {})], sources);
    }
    try {
      userRaw = JSON.parse(text);
    } catch (err) {
      return failure('parse', [mkErr('', 'parse', `invalid JSON in tax profile at ${profilePath}: ${err.message}`, {})], sources);
    }
    ...
    try {
      ...
      throw new Error(`tax loader: cannot read tax-profile schema at ${schemaPath}: ${err.message}`);
    }
