...
import { checkContainment, redact, resolveRoots } from '../containment.mjs';
...
// canonicalized and verified to fall inside an explicit allowlist of roots:
//
...
//
// The mechanism (kernel-order canonicalization, fail-closed root resolution,
// homedir redaction, the accepted TOCTOU residual) lives in lib/containment.mjs
// — factored out by #206 so every options/env → filesystem seam in the plugin
// shares one implementation and one test surface.
...
// not valid JSON`) — and embeds inputs of ~19 bytes or fewer VERBATIM — while
// Node's fs err.message embeds the raw path; neither may be echoed where error
// text can cross an MCP/JSON-RPC boundary. SyntaxErrors reduce to a fixed
// word: V8 exposes no structured position property, and recovering one by
// scanning err.message would re-emit the file's own bytes whenever the content
// contains a position-shaped substring (attacker-controlled digits disguised
// as a genuine parse position). fs errors keep only the errno code (strict
// uppercase shape — OS-derived, never file content); anything else degrades to
// a fixed word. Additive to redact() above, which still home-masks the paths
// the composed messages echo.
function describeError(err) {
  if (err instanceof SyntaxError) return 'parse error';
  // The regex pins the errno SHAPE, not membership: safe only while callers feed
  // fs SystemErrors (kernel-derived codes) — never reuse it on a content-derived `.code`.
...
function failure(kind, errors, sources) {
  return Object.freeze({
    ...
    error: Object.freeze({
      ...
      message:
        kind === 'schema'
          // Content hygiene (#225): this top-level message crosses the
          // MCP/JSON-RPC boundary when index.mjs's rawProfile() re-throws it. The
          // old form echoed `errors[0].path`, which embeds the offending PROPERTY
          // NAME verbatim — the `additionalProperties` branch's `${path}/${key}`,
          // or a deeper additionalProperties-subschema recursion path — and a JSON
          // key can carry secret-shaped bytes. Reduce it to the content-free fact
          // (error count + kind), dropping the path entirely, mirroring #207's
          // `depth`/`tooDeep()` carve-out; redact() is composed on top like every
          // sibling failure site. Direct callers keep the full, unredacted
          // path/message/params for every violation in `error.errors[]`.
          ? redact(`tax profile failed schema validation (${errors.length} error(s))`)
          : errors[0]
            ? errors[0].message
...
export function loadProfile(options = {}) {
  ...
  // canonicalize into one of these roots. resolveRoots drops a root that cannot
  // be canonicalized (it can vouch for nothing); if BOTH roots drop, every read
  // is refused — note the mode change: even the bundled defaults then surface
  // as a structured `containment` failure instead of the packaging-invariant
  // throw below. Fail-closed, and effectively unreachable (assets/tax ships
  // alongside this module). checkContainment's failure detail is already
  // homedir-redacted end-to-end (message, path, roots — see lib/containment.mjs),
  // so nothing here can leak the OS username across an MCP/JSON-RPC boundary.
  const roots = resolveRoots([dataDir, ASSETS_TAX]);
  const containmentFailure = (label, p) => {
    const detail = checkContainment(label, p, roots);
    if (detail === null) return null;
    return failure(
      'containment',
      [mkErr('', 'containment', detail.message, { path: detail.path, roots: detail.roots })],
      sources,
    );
  };
  ...
  if (existsSync(profilePath)) {
    ...
    try {
      ...
      // Content hygiene (#207): a fixed token only — V8's SyntaxError.message
      // quotes raw bytes of the offending file, so it never rides the envelope.
  ...
  try {
    ...
    if (err instanceof RangeError) {
      // Content hygiene (#207): tooDeep()'s message can name the offending JSON
      // key path — profile-derived content (`overrides` / `scheduleLineMap` are
      // schema-open, so their nested key names are attacker-influenceable) that
      // must never ride the envelope. Reduce to the content-free fact, with
      // redact() composed on top like every sibling site. Direct callers of
      // resolveProfile / deepMerge still get the full RangeError path.
      return failure(
        'depth',
        [mkErr('', 'maxDepth', redact(`tax profile nesting exceeds the maximum supported depth of ${MAX_DEPTH}`), {})],
        sources,
      );
    }
    throw err;
