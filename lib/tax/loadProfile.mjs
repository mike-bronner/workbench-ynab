...
// A content-free suffix for a filesystem/parse error in a failure message.
// Node's free-form `err.message` is NEVER echoed on a failure path: for a
// JSON.parse SyntaxError it quotes a snippet of the offending file's raw bytes
// (up to ~10 chars — a secret-shaped non-JSON file would leak them straight into
// the structured failure the facade (lib/tax/index.mjs) re-throws across an
// MCP/JSON-RPC boundary; issue #207). We surface only `err.code` — the fixed
// errno string (ENOENT/EACCES/…) an io error carries — which names the failure
// class without ever carrying file content. A SyntaxError has no `code`, so a
// parse failure reduces to the caller's fixed, content-free prefix.
function errorCodeSuffix(err) {
...
export function loadProfile(options = {}) {
  ...
  try {
    ...
    // Never echo err.message: this try wraps JSON.parse too, so a non-JSON
    // defaultsPath (reachable via options.defaultsPath) makes err a SyntaxError
    // whose message quotes the file's bytes (#207). errorCodeSuffix carries only
    // the content-free errno; redact still masks the path.
    throw new Error(redact(`tax loader: cannot read bundled default ruleset at ${defaultsPath}${errorCodeSuffix(err)}`));
  }
  ...
  if (existsSync(profilePath)) {
    ...
    try {
      ...
      // Never echo err.message; errorCodeSuffix carries only the content-free
      // errno (ENOENT/EACCES/…) and redact masks the path (#207).
      return failure('io', [mkErr('', 'io', redact(`cannot read tax profile at ${profilePath}${errorCodeSuffix(err)}`), {})], sources);
    }
    try {
      userRaw = JSON.parse(text);
    } catch {
      // V8's JSON.parse SyntaxError.message quotes a snippet of the offending
      // file's raw bytes — a secret-shaped non-JSON profile would leak them into
      // the failure the facade re-throws across an MCP/JSON-RPC boundary (#207).
      // Report only the fixed, content-free description; redact masks the path.
      return failure('parse', [mkErr('', 'parse', redact(`invalid JSON in tax profile at ${profilePath}`), {})], sources);
    }
    ...
    try {
      ...
      // Same content hygiene as the defaults read above: this wraps JSON.parse,
      // so a non-JSON schemaPath (via options.schemaPath) would make err a
      // SyntaxError quoting file bytes. Surface only the content-free errno (#207).
      throw new Error(redact(`tax loader: cannot read tax-profile schema at ${schemaPath}${errorCodeSuffix(err)}`));
    }
