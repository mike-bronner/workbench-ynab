...
//   The one exception is the config PATH itself (#169 → #206): a resolved
//   config path escaping the data-dir root is an illegitimate request, not a
//   user error, and is refused with a structured `containment` throw before
//   any read (see lib/containment.mjs) — never silently absorbed into the
//   defaults, where the violation would go unnoticed.
//
// PURITY
 ...
 * or invalid values all degrade to the defaults — never a throw. The config
 * PATH is the exception (#206): a resolved path escaping the data-dir root
 * (`options.dataDir` → env `YNAB_DATA_DIR` → the canonical plugin-data dir)
 * throws a structured `containment` error before any read.
 *
 ...
 *   root (test seam), mirroring loadProfile/loadTracker.
...
export function loadThresholds(options = {}, env = process.env) {
  ...
  // Path containment (#169 → #206): the data dir names the allowlist root —
