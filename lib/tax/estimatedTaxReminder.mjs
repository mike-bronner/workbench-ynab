...
// Strict YYYY-MM-DD → UTC epoch-day integer, or null. The parse (including the
// calendar check that rejects impossible dates like 2026-02-30) lives in the ONE
// shared parser — civilDate.mjs — so no seam can re-derive a shape-only regex and
// silently drop it (#263). UTC is used purely as a stable civil-date frame (no tz
// influence): both operands go through the same frame, so the subtraction is an
