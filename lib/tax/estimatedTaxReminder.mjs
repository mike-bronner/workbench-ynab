...
// epochDay() — the strict civil-date parser — now lives in ./civilDate.mjs, so
// the tax engine's other date seams reuse the calendar check instead of
// re-deriving a shape-only regex that accepts impossible dates (#240, #263).
//
// One deliberate NARROWING came with the move (#263): the shared parser anchors
// the match at BOTH ends, where this module's private version matched a prefix.
// So a date carrying a suffix — '2025-04-15T00:00:00Z', '2025-04-155' — used to
// parse as April 15 and now returns null. Nothing in this repo can supply one:
// every civil date is built date-only (loadProfile's zero-padded
// `${year}-${mm}-${dd}`, the facade's `toISOString().slice(0, 10)`, YNAB's
// date-only `tx.date`), and refusing a shape the caller did not mean is the
// fail-closed reading. Rejections are otherwise identical.
