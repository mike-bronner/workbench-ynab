...
//     //    ytdData — plus an explicit ytdData.asOfDate ('YYYY-MM-DD'). It THROWS
//     //    when any is missing, rather than emitting a silently-wrong summary (e.g.
//     //    a 'NaN-01-15' quarterly due date) or anchoring on the host clock.
 ...
 *   due date". No default: computeTaxSummary THROWS when it is missing or malformed,
 *   rather than reading the host clock (which would anchor the summary on the wrong
 *   day, or the wrong tax year, near midnight or outside UTC).
 ...
 * supply `asOfDate` — the engine never reads the host clock for it.
 *
 * @param {object} profile loadEffectiveProfile() result.
 * @param {YtdData} [ytdData] pre-aggregated YTD figures (all dollars); must carry
 *   a 'YYYY-MM-DD' `asOfDate`.
 * @returns {TaxSummary} the Section-12 summary.
 * @throws {Error} when `asOfDate` is missing or not a 'YYYY-MM-DD' string, or when
 *   neither the profile nor `ytdData` resolves a taxYear + filingStatus.
 */
export function computeTaxSummary(profile, ytdData = {}) {
  ...
  // read. The strict 'YYYY-MM-DD' shape is required, not cosmetic: the due-date
  // search below (`d.date >= asOfDate`) is a LEXICOGRAPHIC string compare, so any
  // other shape ('5/1/2025', a Date, a number) compares wrong and would surface a
  // silently-incorrect next payment rather than an error.
  if (typeof asOfDate !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(asOfDate)) {
    throw new Error(
      "computeTaxSummary requires an explicit ytdData.asOfDate ('YYYY-MM-DD' civil " +
        `date); got ${JSON.stringify(asOfDate)}. Supply the caller's own resolved ` +
        'date — the engine never reads the host clock, which would anchor the summary ' +
        'on the wrong day (or wrong tax year) near midnight or outside UTC.',
    );
  }
