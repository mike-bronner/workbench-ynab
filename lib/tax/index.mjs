...
// object and return the bare resolved profile the M3-4 classifier expects. The
// result carries the resolved profile under `.profile`; a bare profile is used
// as-is. Null/undefined passes through unchanged.
function rawProfile(profile) {
  return profile && profile.profile ? profile.profile : profile;
}
...
export function computeTaxSummary(profile, ytdData = {}) {
  ...
  const thresholdPercent = profile.getThreshold('medicalAgiPercent') ?? 0.075;
  ...
  const seTaxRate = profile.getThreshold('seTaxRate') ?? 0.153;
  const seTax = {
