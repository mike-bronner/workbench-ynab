...
export function saveTracker(state, options = {}) {
  ...
  // mode 0o700 applies to the LEAF dir created here; a pre-existing parent (e.g.
  // ~/.claude/plugins/data) keeps its own mode. The data itself stays safe (file
  // 0600, leaf 0700) — only directory LISTING of a shared parent could leak, and
  // force-chmod-ing parents we don't own would overreach, so we don't.
