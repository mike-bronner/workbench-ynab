...
//   computeEstimate, quarterlyEstimate, quarterForDate, isEstimatedTaxPayment,
 ...
 * each into the tracker's payment record, attributed to the quarter its date
 * falls in. Skips transactions with no usable id or date (they cannot be
 * deduped or attributed).
 *
...
export function detectPayments(transactions, matchers, dueDates) {
  ...
  for (const raw of Array.isArray(transactions) ? transactions : []) {
    ...
    const quarter = quarterForDate(tx.date, dueDates);
    if (!quarter) continue;
    out.push({
      date: tx.date,
...
export function upsertQuarterEstimate(state, { year, quarter, estimate } = {}) {
  ...
  entry.estimated_liability = cents(estimate?.quarterLiability ?? estimate?.totalLiability ?? 0);
