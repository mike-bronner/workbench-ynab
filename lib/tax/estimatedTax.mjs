...
//   — and, exactly like loadProfile, both seams are path-contained (#169 → #206):
//   the resolved tracker path must canonicalize into the resolved data dir, or
//   the read/write is refused with a structured `containment` throw (see
//   lib/containment.mjs). The write seam is the sharper edge: an unchecked
...
import { readFileSync, existsSync, mkdirSync, chmodSync, writeFileSync, renameSync, rmSync } from 'node:fs';
import { homedir } from 'node:os';
import { dirname, join } from 'node:path';

import classify from './classifyTransaction.mjs';
import { parseCivilDate } from './civilDate.mjs';
 ...
 *
 * Dates are parsed by the ONE shared strict parser (civilDate.mjs), so an
 * impossible-but-well-shaped date ('2025-13-45', '2025-02-30') returns null
 * instead of falling through to the month mapping and answering plausibly but
 * wrongly (#263).
 *
...
export function quarterForDate(dateISO, dueDates) {
  ...
  const point = md(civil.month, civil.day);
  ...
  if (civil.month <= 3) return 1;
  if (civil.month <= 5) return 2;
  if (civil.month <= 8) return 3;
  return 4;
}

/**
 * Attribute a PAYMENT date to both the estimated-tax quarter it pays toward AND
 * the tax YEAR that quarter belongs to, by the IRS DUE-DATE schedule (each
 * quarter's `month`/`day` in the profile's quarterlyEstimatedDueDates) — NOT the
 * income-earning window. A payment belongs to the quarter whose due date is the
 * first due date on or after the payment date — i.e. the quarter it is *paying
 * toward* — INCLUDING the year-end rollover where a payment on or before Q4's due
 * date (Jan 15 of the next year) attributes to that prior tax year's Q4.
 *
 * Returns `{ quarter, taxYearOffset }` where `taxYearOffset` is added to the
 * payment's own calendar year to get the tax year it belongs to: 0 for every
 * in-year payment, and -1 for the post-year-end portion of the wrapping Q4 window
 * (a Jan 1–15 payment toward the PRIOR tax year's Q4). Callers add the offset to
 * the payment date's calendar year — keeping the year math here, where the wrap
 * is detected, instead of re-deriving it downstream. Returns null when the
 * schedule carries no usable due dates.
 *
 * Dates are parsed by the ONE shared strict parser (civilDate.mjs), so an
 * impossible-but-well-shaped date ('2025-13-45', '2025-02-30') returns null
 * instead of feeding garbage month/day values into the quarter-window
 * comparison (#263).
 *
 ...
 */
// The quarterly schedule as a comparable cycle: only entries carrying month, day
// and quarter, ordered by quarter. Quarters are labelled 1–4 in chronological cycle
// order (Apr→Jun→Sep→Jan-next), so sorting by quarter gives the cycle. Each quarter
// owns the window that ENDS at its due date: (prevDue, thisDue].
function normalizeSchedule(dueDates) {
  return (Array.isArray(dueDates) ? dueDates : [])
    .filter((d) => d && d.month != null && d.day != null && d.quarter != null)
    .map((d) => ({ quarter: d.quarter, month: d.month, day: d.day, dueMd: md(d.month, d.day) }))
    .sort((a, b) => a.quarter - b.quarter);
}

/**
 * The tax year's FINAL payment: the quarter whose due date lands in calendar year
 * N+1 even though the quarter belongs to tax year N (the US federal Q4, due Jan 15).
 *
 * Identified structurally against the FIRST quarter in cycle order: a quarter whose
 * due date falls earlier in the calendar than Q1's has rolled past the year end. Of
 * those, the highest quarter number is the year's last payment — the date the prior
 * tax year finally closes. Returns null when the schedule is empty or every due date
 * sits inside its own tax year.
 *
 * NOT the same question as `attributePaymentDate`'s per-window "does this window
 * cross the year end" test: the Q4→Q1 step of ANY schedule crosses the year end
 * (Dec 31 → Mar 31 next year), while a schedule can still have no due date that
 * rolls over. The two tests are deliberately separate.
 *
 * @param {Array} dueDates profile.quarterlyEstimatedDueDates (month/day per quarter).
 * @returns {?{quarter: number, month: number, day: number, dueMd: number}} the rolled-over
 *   entry, or null when none rolls over.
 */
export function wrappingQuarterDue(dueDates) {
  const schedule = normalizeSchedule(dueDates);
  if (schedule.length === 0) return null;
  const first = schedule[0];
  const rolled = schedule.filter((d) => d.dueMd < first.dueMd);
  return rolled.length === 0 ? null : rolled[rolled.length - 1];
}

function attributePaymentDate(dateISO, dueDates) {
  const civil = parseCivilDate(dateISO);
  if (!civil) return null;
  const point = md(civil.month, civil.day);
  const schedule = normalizeSchedule(dueDates);
  ...
  for (let i = 0; i < schedule.length; i += 1) {
    ...
    const wraps = prev.dueMd > cur.dueMd; // this window crosses the year end
    if (wraps) {
      // The wrapping window spans two calendar years. A date AFTER the prior due
      // date is late in the SAME calendar year as the tax year (offset 0); a date
      // on/before this quarter's due date is in the NEXT calendar year but pays
      // toward the PRIOR tax year (offset -1 — the Jan-15 → prior-year-Q4 case).
      if (point > prev.dueMd) return { quarter: cur.quarter, taxYearOffset: 0 };
      if (point <= cur.dueMd) return { quarter: cur.quarter, taxYearOffset: -1 };
    } else if (point > prev.dueMd && point <= cur.dueMd) {
      return { quarter: cur.quarter, taxYearOffset: 0 };
    }
  }
  return null;
}
...
/**
 * Which estimated-tax quarter (1–4) a PAYMENT date attributes to — the quarter
 * half of {@link attributePaymentDate}. Distinct from quarterForDate, which
 * attributes INCOME by its earning period: the quarter an income transaction
 * belongs to is governed by when it was earned; the quarter a payment belongs to
 * is governed by when it was due. Returns null when the schedule carries no
 * usable due dates, or when `dateISO` is not a real calendar date (#263).
 *
 * @param {string} dateISO  YYYY-MM-DD payment date.
 * @param {Array}  dueDates profile.quarterlyEstimatedDueDates (month/day per quarter).
 */
export function quarterForPaymentDate(dateISO, dueDates) {
  const attribution = attributePaymentDate(dateISO, dueDates);
  return attribution ? attribution.quarter : null;
}
 ...
 * each into the tracker's payment record, attributed by the DUE-DATE schedule
 * (attributePaymentDate, not the income window) to BOTH the quarter it pays
 * toward and the tax `tax_year` that quarter belongs to. The `tax_year` is the
 * payment's own calendar year, shifted back one for the post-year-end rollover
 * (a Jan 1–15 payment toward the PRIOR tax year's Q4) — so reconcilePayments can
 * file a payment into the year it actually belongs to even when this year's pull
 * spills across the calendar boundary. Skips transactions with no usable id or
 * date (they cannot be deduped or attributed).
 *
 ...
 * @returns {Array<{date:string, amount_usd:number, ynab_transaction_id:string, quarter:number, tax_year:number}>}
 */
export function detectPayments(transactions, matchers, dueDates) {
  ...
  for (const raw of Array.isArray(transactions) ? transactions : []) {
    ...
    const attribution = attributePaymentDate(tx.date, dueDates);
    ...
    out.push({
      ...
      tax_year: Number(tx.date.slice(0, 4)) + attribution.taxYearOffset,
    });
  }
  return out;
}
...
// Containment allowlist for the tracker file (#169 → #206): the resolved data
// dir — options.dataDir → env YNAB_DATA_DIR → the canonical plugin-data dir.
// Naming a root is an embedding-level trust decision (the test seam), so an
// explicit dataDir joins the allowlist WITHOUT widening the default no-options
// surface; an explicit trackerPath never vouches for itself. An unresolvable
// root is dropped by resolveRoots (fail closed — every access then refused).
function trackerRoots(options = {}) {
  const env = options.env ?? process.env;
 ...
 * recorded payment history, and a tracker path escaping the data-dir root
 * throws a structured `containment` error before any read (#206).
 */
export function loadTracker(options = {}) {
  const path = trackerPathFor(options);
  // Checked before existsSync (#206): an escaping tracker path is an
  // illegitimate REQUEST — refusing it outright (rather than only when the
  // target happens to exist) keeps the failure deterministic and avoids acting
  // as an existence oracle for paths outside the root.
  assertContained('the tax tracker', path, trackerRoots(options));
  ...
  // Validation is intentionally shallow (top-level `years` only): nested numeric
  // fields are tolerated and `Number(...)`-coerced at use. The file is written
  // 0600, so a malformed nested value requires the owner to hand-edit their own
  // tracker — not a trust boundary worth a deep type-checker against.
  return parsed;
}
...
export function saveTracker(state, options = {}) {
  ...
  // path.
  assertContained('the tax tracker', path, trackerRoots(options), 'write');
  // Create the LEAF dir 0700. mkdirSync's `mode` applies ONLY when it creates the
  // dir — a no-op if it already exists — so a leaf left 0755 by an earlier writer
  // would stay world-traversable. chmodSync re-tightens it unconditionally, so the
  // tracker's dir holds the invariant exactly as lib/monitor/alerts.mjs and
  // bin/audit-log.sh do for this same data class (they chmod the dir on every
  // write). A pre-existing PARENT (e.g. ~/.claude/plugins/data) keeps its own mode;
  // force-chmod-ing dirs we don't own would overreach, so we stop at the leaf.
  ...
  // lib/monitor/state.mjs writeState.
...
/**
 * Reconcile detected estimated-tax payments into the tracker for ONE tax year.
 * Only payments whose `tax_year` matches `year` are filed — a payment that
 * belongs to a different tax year (e.g. a Jan 1–15 payment toward the PRIOR
 * year's Q4, or a next-year payment that surfaced in this year's unbounded
 * transaction pull) is skipped here and recorded when ITS year is reconciled, so
 * one year's run never contaminates another's. Each kept payment lands in the
 * quarter its date attributes to; a payment whose ynab_transaction_id is already
 * recorded for that quarter is skipped (idempotent — re-running never
 ...
 * @param {object} args
 * @param {number|string} args.year
 * @param {Array}  args.payments detectPayments() output ({…, quarter, tax_year}).
 */
export function reconcilePayments(state, { year, payments = [] } = {}) {
  ...
  for (const p of Array.isArray(payments) ? payments : []) {
    ...
    // Bound to the requested tax year. A payment carrying a different tax_year
    // belongs to another year's run; older callers that omit tax_year are not
    // filtered (back-compat).
    if (p.tax_year != null && String(p.tax_year) !== wanted) continue;
