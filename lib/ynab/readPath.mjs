...
//
// `scheduled_transactions` is a real cached resource as of the 0.27.1 re-vendor
// (#157): upstream registers `ynab_list_scheduled_transactions`, whose response
// keys its rows under `scheduled_transactions` and paginates offset-based like
// every other list tool — so it needs no adapter of its own, only this entry and
// a `call` that routes the resource string to that tool. Before 0.27.1 the
// vendored bundle had no read path to them at all (docs/ynab-read-path.md §1).
...
// Default page size, matched to the `limit` default (50) of the five original
// vendored list tools (accounts, categories, payees, transactions, months);
// `list_budgets` returns everything at once. `list_scheduled_transactions`
// (added by the 0.27.1 re-vendor) defaults to `limit: 100` instead — the mismatch
// is inert because every list response carries an explicit `next_offset` whenever
// `has_more` is true, so the walk follows the server's own offset and never falls
// back to this constant. See docs/ynab-read-path.md §1.
...
// response (@dizzlkheinz/ynab-mcpb@0.27.1): rows are keyed by the resource name
// (`{ transactions: [...] }`, `{ accounts: [...] }`,
// `{ scheduled_transactions: [...] }`, …), and continuation is OFFSET-based —
// `has_more` is true while rows remain and `next_offset` is the offset of the
// following page. There is NO cursor. `next_offset` is absent on the last page of
// the five original list tools and explicitly `null` on the last page of
// `list_scheduled_transactions`; both terminate here, because `has_more` is what
// stops the walk and it is read first. A transport with a different shape injects
// its own getItems/getNextParams; the driver itself stays transport-agnostic. See
// docs/ynab-read-path.md §1.
 ...
 * resource exactly once — paginated in full and rate-limit-retried — and
 * memoizes the IN-FLIGHT promise (not just the resolved entry), so concurrent
 * calls for the same resource+params share one underlying pull and every later
 * call returns the cached rows without touching the MCP (#158). On exhausted
 * retries it degrades: the resource+params pull is recorded as partial,
 * `annotationFor(resource, params)` yields the
 * `[YNAB rate limit hit — partial review]` label, and `get` returns
 * whatever rows were collected with `partial: true` rather than throwing.
 * Because the in-flight promise is what's memoized, `stats().resources`
 * (`cache.size`) counts a pull from the moment it starts: under concurrent
 * same-key access it includes in-flight pulls, not only settled entries. In
 * the sequential fetch-then-derive flow every `get` is awaited before
 * `stats()` is read, so the count reflects settled entries as before (#199).
 * @param {(resource: string, params: object) => Promise<object>} call
 * @param {object} [opts] retry + pagination + adapter options.
 * @returns {{
...
export function createReadCache(call, opts = {}) {
  ...
  async function fetchEntry(key, resource, params) {
    try {
      ...
      return Object.freeze({ items, pages, partial: false, serverKnowledge });
    } catch (err) {
  ...
  async function get(resource, params = {}) {
    ...
    // Memoize the IN-FLIGHT promise, synchronously, before collectAllPages ever
    // resolves — not the resolved entry after the await. Two concurrent get()
    // calls for the same key then share one underlying pull, so a divergent
    // later outcome (e.g. a 429-degraded partial) can never clobber `cache` or
    // `degraded` for a key that already resolved (#158, fetch-once AC #2).
    const pending = fetchEntry(key, resource, params);
    cache.set(key, pending);
    // A non-rate-limit failure (fetchEntry's rethrow) was never memoized before
    // promise-caching either: evict it so a later sequential get() may retry,
    // exactly as pre-#158. Rate-limit exhaustion RESOLVES (degraded partial),
    // so degraded entries stay cached as before.
  ...
  return {
    ...
    // `resources` counts in-flight pulls too: cache.size includes entries
    // memoized synchronously at get() start, before the pull settles (#199).
