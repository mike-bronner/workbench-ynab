...
# {{output_path}} scalar slot (32 KB of '&' ≈ 2 min). `${#out_dir}` is O(1),
# and 1024 bytes is PATH_MAX on macOS — a longer path could never be created
# anyway — so refuse loudly naming the field instead of pegging the CPU on the
# way to a cryptic ENAMETOOLONG.
[ "${#out_dir}" -le 1024 ] || usage_err "output dir is longer than 1024 bytes after expansion — check .report.output_dir"
